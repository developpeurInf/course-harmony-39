...
USING (
  auth.uid() IN (
    SELECT professor_id FROM public.courses WHERE id = exercise.course_id
  )
);
...
USING (
  auth.uid() IN (
    SELECT professor_id FROM public.courses WHERE id = exam.course_id
  )
);
...
USING (
  auth.uid() IN (
    SELECT professor_id FROM public.courses WHERE id = enrollment.course_id
  )
);
...
USING (
  bucket_id = 'course-materials' AND (
    -- Professors can access all their course materials
    auth.uid() IN (
      SELECT professor_id FROM public.courses 
      WHERE courses.pdf_url LIKE '%' || name || '%'
         OR id::text IN (
           SELECT course_id FROM public.exercises WHERE exercises.pdf_url LIKE '%' || name || '%'
           UNION
           SELECT course_id FROM public.exams WHERE exams.pdf_url LIKE '%' || name || '%'
         )
    )
    OR
    ...
    auth.uid() IN (
      ...
      WHERE course_id IN (
        SELECT id FROM public.courses WHERE courses.pdf_url LIKE '%' || name || '%'
        UNION
        SELECT course_id FROM public.exercises WHERE exercises.pdf_url LIKE '%' || name || '%'
        UNION 
        SELECT course_id FROM public.exams WHERE exams.pdf_url LIKE '%' || name || '%'
